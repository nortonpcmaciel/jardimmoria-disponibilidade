...
O arquivo inicial não contém datas individuais de vendas e cancelamentos. Por isso, a série importada permite o comparativo mensal geral, mas não reconstrói o histórico por objeto ou por quadra. Esse detalhamento individual começa na primeira alteração salva com a versão atual. Os totais atuais continuam disponíveis desde a primeira leitura. A informação é quantitativa: não há preços ou valores financeiros na fonte.

O comparativo geral do dashboard também carrega `data/dashboard-historico.json`, preenchido com a planilha histórica de 01/2025 a 08/2026 e com o movimento inicial informado para 09/2026. O campo `eventsIncludedThroughRevision` define o corte já incorporado nessa base; apenas eventos de revisões posteriores são somados, evitando duplicidade. Como a planilha não identifica quadras, o filtro por quadra usa somente os eventos individuais registrados pelo sistema. No quadro de distribuição, Casas Populares e Área Verde são omitidas e os percentuais usam apenas as situações apresentadas como denominador.
