...
test('GitHub Pages: leitura pública, escrita autenticada, acentos e conflitos', async () => {
  ...
  await assert.rejects(store.save({ id: '1', situacao: 'DISPONÍVEL', revision: 0 }, '', original.dataset), /token/);
  const updated = await store.save({ id: '1', situacao: 'DISPONÍVEL', revision: 0 }, 'teste-token', original.dataset);
  ...
  await assert.rejects(store.save({ id: '2', situacao: 'QUITADO', revision: 0 }, 'teste-token', original.dataset), /Outro usuário/);
  await assert.rejects(store.save({ id: '2', situacao: 'QUITADO', revision: 1 }, 'teste-token', 'outra-camada'), /camada/);
  await assert.rejects(store.save({ id: '2', situacao: 'INVALIDA', revision: 1 }, 'teste-token', original.dataset), /inválidos/);
  await store.save({ id: '1', situacao: 'DISPONÍVEL', revision: 1 }, 'teste-token', original.dataset);
  ...
  await assert.rejects(concurrent.save({ id: '1', situacao: 'QUITADO', revision: 1 }, 'teste-token', original.dataset), /Outro usuário/);
});
