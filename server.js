...
function createServer({ token, statePath = path.join(ROOT, 'private/situacoes.json'), now = () => new Date() } = {}) {
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (!/^\/(index.html|index2.html|atualizar.html|dashboard.html|data\/(?:situacoes|dashboard-historico|lotes-detalhes)\.json|(?:resources|layers|styles|images|webfonts)\/[\w./ -]+)$/.test(pathname) || pathname.split('/').includes('..'))
      ...
      const types = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.json': 'application/json; charset=utf-8', '.css': 'text/css', '.png': 'image/png', '.svg': 'image/svg+xml', '.woff2': 'font/woff2' };
